          ...
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-0.5">
                ${Number(activeLoan.amount).toLocaleString()} requested
                {activeLoan.approved_amount && ` · $${Number(activeLoan.approved_amount).toLocaleString()} approved`}
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Loan Amount (USD) <span className="text-gray-400 text-xs">max ${maxAmt}</span>
                </label>
                <input type="number" min="1" max={maxAmt} step="1"
                  value={form.amount}
                  onChange={(e) => setForm((f) => ({ ...f, amount: e.target.value }))}
                  placeholder={`Up to $${maxAmt}`}
              ...
              <div className="bg-indigo-50 rounded-xl p-4 border border-indigo-100">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-center text-sm">
                  <div className="bg-white rounded-lg p-3">
                    <div className="font-bold text-gray-900">${Number(form.amount).toLocaleString()}</div>
                  ...
                  <div className="bg-white rounded-lg p-3">
                    <div className="font-bold text-indigo-600">${emi.toFixed(2)}/mo</div>
                    <div className="text-xs text-gray-400 mt-0.5">Monthly EMI</div>
                  </div>
                  <div className="bg-white rounded-lg p-3">
                    <div className="font-bold text-gray-900">${totalPayback.toFixed(2)}</div>
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="font-medium text-gray-800 text-sm">
                        ${Number(l.amount).toLocaleString()} requested
                      </span>
                      {l.approved_amount && l.approved_amount !== l.amount && (
                        <span className="text-xs text-green-600">→ ${Number(l.approved_amount).toLocaleString()} approved</span>
                      )}
