...
import { Coins, AlertCircle, CheckCircle, Clock, Calculator, XCircle } from "lucide-react";
...
export default function LoanApplication() {
  ...
  const load = async () => {
    try {
      const [s, l] = await Promise.all([api.get("/score"), api.get("/loan/status")]);
      setScoreData(s.data);
      setLoans(l.data);
  ...
  const scoreFloor = scoreData?.scoreFloorForLoan ?? 60;

  const activeLoan = loans.find(l => ["pending","approved","repayment_requested"].includes(l.status));
  const emi = form.amount && rate ? calcEMI(Number(form.amount), rate, Number(form.duration_days)) : 0;
    ...
    <div className="max-w-3xl mx-auto px-4 py-10">
          ...
          <div className="font-semibold text-gray-900">
            ...
            <span className={`ml-2 text-xs px-2 py-0.5 rounded-full capitalize
              ${tier === "low" ? "bg-green-200 text-green-800" :
                ...
                tier === "high" ? "bg-red-100 text-red-700" : "bg-gray-100 text-gray-500"}`}>
              {tier} tier
            </span>
          </div>
          {eligible
            ? <p className="text-sm text-gray-600 mt-1">
                You qualify for up to <strong>{formatINR(maxAmt)}</strong> at <strong>{rate}% interest</strong>.
                Loans are reviewed and approved by a lender.
              </p>
            : <p className="text-sm text-gray-500 mt-1">
                Score below {scoreFloor}. Submit and verify more activities to qualify.
              </p>
          }
        </div>
      </div>
          ...
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-0.5">
                ...
                {activeLoan.approved_amount && ` · ${formatINR(activeLoan.approved_amount)} approved`}
                {activeLoan.interest_rate && ` · ${activeLoan.interest_rate}% interest`}
              </p>
              {activeLoan.lender_note && (
                <p className="text-xs text-indigo-600 mt-1">Lender note: "{activeLoan.lender_note}"</p>
              )}
              {activeLoan.status === "pending" && (
                <p className="text-xs text-gray-400 mt-1">⏳ Awaiting lender review…</p>
              )}
            </div>
            ...
            {activeLoan.status === "repayment_requested" && (
              <p className="text-xs text-purple-600 font-medium">Repayment confirmation pending from lender</p>
            )}
          </div>
        </div>
      )}
      ...
      {eligible && !activeLoan && !submitted && (
        <div className="bg-white border border-gray-100 rounded-2xl shadow-sm p-6 mb-6">
          ...
          <form onSubmit={handleApply} className="space-y-4">
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Loan Amount (INR) <span className="text-gray-400 text-xs">max {formatINR(maxAmt)}</span>
                </label>
              ...
              <div>
                ...
                <select value={form.duration_days}
                  ...
                  {[7,14,30,60,90,180,365].map((d) => (
                    <option key={d} value={d}>{d} days ({Math.ceil(d/30)} month{d>30?"s":""})</option>
                  ))}
                </select>
              </div>
            </div>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Purpose <span className="text-gray-400 text-xs">(optional — helps lender approve faster)</span>
              </label>
              <input type="text"
                value={form.purpose}
              ...
              <div className="bg-indigo-50 rounded-xl p-4 border border-indigo-100">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-center text-sm">
                  <div className="bg-white rounded-lg p-3">
                    <div className="font-bold text-gray-900">{formatINR(form.amount)}</div>
                    <div className="text-xs text-gray-400 mt-0.5">Principal</div>
                  </div>
                  <div className="bg-white rounded-lg p-3">
                    <div className="font-bold text-gray-900">{rate}%</div>
                    <div className="text-xs text-gray-400 mt-0.5">Annual Rate</div>
                  </div>
                  <div className="bg-white rounded-lg p-3">
                    <div className="font-bold text-indigo-600">{formatINR(emi, 2)}/mo</div>
                    <div className="text-xs text-gray-400 mt-0.5">Monthly EMI</div>
                  </div>
                  <div className="bg-white rounded-lg p-3">
                    <div className="font-bold text-gray-900">{formatINR(totalPayback, 2)}</div>
                    <div className="text-xs text-gray-400 mt-0.5">Total Payback</div>
                  </div>
                </div>
                <p className="text-xs text-indigo-500 mt-2">
                  * Final terms may be adjusted by the lender at time of approval.
                </p>
              </div>
            )}
      ...
      {submitted && (
        <div className="bg-green-50 border border-green-200 rounded-2xl p-6 mb-6 text-center">
          ...
          <h2 className="font-bold text-lg text-gray-900 mb-1">Application Submitted!</h2>
          <p className="text-sm text-gray-500">A lender will review your application and respond shortly. Check the status below.</p>
        </div>
      )}
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {l.approved_amount && l.approved_amount !== l.amount && (
                        <span className="text-xs text-green-600">→ {formatINR(l.approved_amount)} approved</span>
                      )}
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium capitalize flex items-center gap-1 ${STATUS_COLORS[l.status] || ""}`}>
                    ...
                    {l.lender_note && l.status === "approved" && (
                      <p className="text-xs text-indigo-600 mt-1">Lender note: "{l.lender_note}"</p>
                    )}
                  </div>
                  {l.status === "approved" && (
                    <button onClick={() => requestRepayment(l.id)}
