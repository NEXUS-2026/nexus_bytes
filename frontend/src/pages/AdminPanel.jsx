...
export default function AdminPanel() {
  ...
  useEffect(() => {
    Promise.all([api.get("/activity"), api.get("/loan/status")])
      .then(([a, l]) => {
  ...
  const stats = [
    ...
    {
      label: "Verified",
      value: activities.filter((a) => a.status === "verified").length,
    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        {stats.map((s) => (
          ...
          >
            <div
              className={`w-9 h-9 rounded-xl flex items-center justify-center mb-3 ${colors[s.color]}`}
            >
              {s.icon}
            </div>
        ...
        <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 text-gray-500 text-xs font-medium">
              <tr>
                {["ID", "User", "Title", "Category", "Status", "Submitted"].map(
                  (h) => (
              ...
              {activities.map((a) => (
                <tr key={a.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-5 py-3">
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full ${catColor[a.category] || ""}`}
                    >
                      {a.category}
                    </span>
                  </td>
            ...
            <tbody className="divide-y divide-gray-50">
              {loans.map((l) => (
                <tr key={l.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-5 py-3">
                    {l.approved_amount ? formatINR(l.approved_amount) : "—"}
                  </td>
                  <td className="px-5 py-3 capitalize">{l.tier}</td>
                  <td className="px-5 py-3">{l.interest_rate}%</td>
                  <td className="px-5 py-3">
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full font-medium ${statusColor[l.status] || ""}`}
                    >
                      {l.status}
                    </span>
                  </td>
